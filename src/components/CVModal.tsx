...
export default function CVModal({
  ...
  const handleDownload = async (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    try {
      const response = await fetch("/Ben_Hankins_SA_Resume.pdf");
      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.download = "Ben_Hankins_SA_Resume.pdf";
      ...
      window.open("/Ben_Hankins_SA_Resume.pdf", "_blank");
    }
  };
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-4xl h-[95vh] md:h-[90vh] flex flex-col overflow-hidden ring-1 ring-slate-200/50">
        ...
        <div className="flex-1 overflow-auto min-h-0">
          {isMobile ? (
            ...
            <object
              data="/Ben_Hankins_SA_Resume.pdf"
            ...
            // Desktop: Use iframe
            <iframe
              src="/Ben_Hankins_SA_Resume.pdf#toolbar=0&navpanes=0&scrollbar=1"
